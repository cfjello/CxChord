...
import { ChordMap, ChordMapEntry, ChordMapTable, Conflicts, RootNoteNames } from "./Interfaces";
import * as _ from "lodash"
	...
	export function getNoteNumber ( note: string ): number {
		let octave = 5
		const _oct = note.replace(/^[A-Ga-g]+[#b]*/,'')
		if ( ! _.isEmpty(_oct) ) {
			const _octave = _oct.replace(/[^0-9]+/g, '')
		...
		const noteNum  = noteNames.indexOf(noteName) % 12
	...
	export function getNoteName( note = 0, flatOrSharp = "flat" ): string  {
		...
		note = note % 12
		const noteName = rootNoteNames[flatOrSharp][ ( Math.abs(note < 0 ? note + 12 : note) ) % 12 ]
		return noteName
	}
	
	export function getExtName( nameWithCommas : string ) : string {
		const extName = nameWithCommas.replace( /,/g, "").replace( /-1/g, "-no-root" ).replace( /-5/g, "-no-fifth" )
		return extName
	}
			
	export function getChordName( nameWithCommas : string, root = 0 , bass = 255, flatOrSharp = "flat"  ): string {
		let chordName = getNoteName(root, flatOrSharp )
		const extName   = getExtName(nameWithCommas)
		chordName += extName
		// console.debug("chordName(1):'" + chordName + "'")
		const group = _.isUndefined( chordMap[nameWithCommas] ) ? 2 :  chordMap[nameWithCommas].group 
		bass =  bass == 255 ?  root : bass
		if ( bass !==  root && group != GR.rootLess ) {
			const bassName = getNoteName(bass, flatOrSharp)
			chordName += "/" + bassName
		}
		return chordName
	}
	...
	export function getKnockouts(key: string ): number[] {
		let _key = key
		if ( _.isUndefined( knockouts[_key]) ) {
			_key = _key.split(',')[0]
		}
		return _.isUndefined( knockouts[_key]) ? [] : knockouts[_key] 
	}
...
export class ChordForms {	
	...
	public getInvariants( chordName: string ): string[] {
		...
		if ( _.has(chordMap, chordName) ) {
			const chord:ChordMapEntry  = chordMap[chordName]
			_.forIn(chordMap, function(value: ChordMapEntry, key: string) {
	...
	public buildInversions() {
		const mapWithInv = this.chordMapWithInv
		//
		...
		_.forIn(chordMap, function(value: ChordMapEntry, key) {
			...
			//   
			const chord = new ChordInstance(value.notes)
			const interval = chord.getInversion(0)
			for( let i = 0 ; i < value.notes.length; i++) {	
				const _notes = chord.getInversion(i)	
				//
				// Calculate the new inverted root note 
				// 		
				const rootNote = ( - ( Math.abs(value.root) + interval[i] ) ) % 12
